...
public class ComboTreeNodeCollection : IList<ComboTreeNode>, IList, INotifyCollectionChanged {
    ...
    public ComboTreeNode Add(string name, string text, int index)
    {
        ComboTreeNode item = new ComboTreeNode(name, text,index);
        Add(item);
        return item;
    }
    ...
    /// <summary>
    /// Returns the <see cref="ComboTreeNode"/> with the specified node name.
    /// </summary>
    /// <param name="text">The name to match.</param>
    /// <param name="recurse">Whether to search recursively through all child nodes.</param>
    /// <param name="comparisonType">The type of string comparison performed.</param>
    /// <returns>Combo tree node matches the search name</returns>
    public ComboTreeNode FindNode(string text, bool recurse = false, StringComparison comparisonType = StringComparison.OrdinalIgnoreCase)
    {
        IEnumerator<ComboTreeNode> nodes = recurse ? GetNodesRecursive(this, false) : GetEnumerator();

        while (nodes.MoveNext())
        {
            if (nodes.Current.Name.Equals(text, comparisonType))
            {
                return nodes.Current;
            }
        }
